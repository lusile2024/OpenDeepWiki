...
public static class WorkflowAnalysisTaskAiResponseParser
{
    ...
    public static WorkflowAnalysisTaskAiResult Parse(string content)
    {
        var rawContent = StripCodeFenceAndThinkTags(content.Trim());
        var json = ExtractJsonObject(rawContent);
        if (string.IsNullOrWhiteSpace(json))
        {
            throw new InvalidOperationException("AI 未返回可解析的任务 JSON。");
        }
        ...
        throw new InvalidOperationException("AI 返回的任务结果结构无法解析。");
    }

    private static bool TryDeserialize(string json, out WorkflowAnalysisTaskAiResult result)
    {
        result = new WorkflowAnalysisTaskAiResult();

        try
        {
            var parsed = JsonSerializer.Deserialize<WorkflowAnalysisTaskAiResult>(json, JsonOptions);
            if (parsed is null || string.IsNullOrWhiteSpace(parsed.MarkdownDraft))
            {
                return false;
            }
